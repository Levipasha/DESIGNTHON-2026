...
export default function AdminDashboard() {
  ...
  const fetchParticipants = async () => {
    ...
    try {
      const res = await fetch(`http://localhost:5000/api/admin/participants?search=${participantSearch}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      if (res.ok) {
  ...
  const handleToggleCoupon = async (couponId: string) => {
    try {
      const res = await fetch('http://localhost:5000/api/admin/coupons/toggle', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`
  ...
  const handleCreateCoupon = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const res = await fetch('http://localhost:5000/api/admin/coupons/create', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`
  ...
  const handleMergeTeams = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('http://localhost:5000/api/admin/teams/merge', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`
          ...
          {activeTab === 'participants' && (
            <div className="space-y-6 animate-[fadeIn_0.2s_ease-out] text-left">
              ...
              <div className="glass-panel border-white/5 rounded-2xl p-4 flex flex-col sm:flex-row gap-4 items-center justify-between">
                ...
                <a
                  href="http://localhost:5000/api/admin/export-csv"
                  target="_blank"
